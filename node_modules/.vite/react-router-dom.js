...
// node_modules/react-router/node_modules/resolve-pathname/esm/resolve-pathname.js
...
// node_modules/react-router/node_modules/value-equal/esm/value-equal.js
...
function resolvePathname2(to, from) {
  if (from === void 0)
    from = "";
  var toParts = to && to.split("/") || [];
  var fromParts = from && from.split("/") || [];
  var isToAbs = to && isAbsolute2(to);
  var isFromAbs = from && isAbsolute2(from);
  var mustEndAbs = isToAbs || isFromAbs;
  if (to && isAbsolute2(to)) {
    fromParts = toParts;
  } else if (toParts.length) {
    fromParts.pop();
    fromParts = fromParts.concat(toParts);
  ...
  if (!mustEndAbs)
    for (; up--; up)
      fromParts.unshift("..");
  if (mustEndAbs && fromParts[0] !== "" && (!fromParts[0] || !isAbsolute2(fromParts[0])))
    fromParts.unshift("");
  var result = fromParts.join("/");
  if (hasTrailingSlash && result.substr(-1) !== "/")
    result += "/";
...
var resolve_pathname_default2 = resolvePathname2;
...
function createLocation2(path, state, key, currentLocation) {
  ...
  if (currentLocation) {
    if (!location.pathname) {
      ...
      location.pathname = resolve_pathname_default2(location.pathname, currentLocation.pathname);
    }
  } else {
    if (!location.pathname) {
      location.pathname = "/";
